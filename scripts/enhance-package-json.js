...
function detectProjectType(packageData) {
  ...
  const hasTypeScript = dependencies.typescript || fs.existsSync(path.join(process.cwd(), 'tsconfig.json'));
...
function addWorkflowMetadata(packageData) {
  ...
  if (!packageData.description || packageData.description === '') {
    packageData.description = "Enhanced with Claude Agentic Workflow System for autonomous code quality management";
  }
...
function main() {
  ...
  // Merge dependencies
  const devDepsResult = mergeDependencies(packageData.devDependencies, WORKFLOW_DEPENDENCIES.devDependencies);
  ...
  if (process.argv.includes('--with-optional')) {
    const optionalResult = mergeDependencies(packageData.devDependencies, WORKFLOW_DEPENDENCIES.optionalDependencies);
