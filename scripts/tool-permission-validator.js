...
class ToolPermissionValidator {
  ...
  parseAgentConfig(content) {
    ...
    try {
      ...
      if (!config.tools) {
        ...
        } else if (Array.isArray(config.tools) && config.tools.length > 0 && typeof config.tools[0] === 'object') {
          // Extract tool names from objects
          config.tools = config.tools.map(t => t.name || t).filter(Boolean);
        }
      }

      config.allowedBashCommands = config.allowedBashCommands || this.defaultPermissions.allowedBashCommands;
      config.allowedMcpServers = config.allowedMcpServers || this.defaultPermissions.allowedMcpServers;
  ...
  isToolAllowed(tool, agentConfig, context) {
    ...
    if (tool.startsWith('bash:') || tool.startsWith('npm ') || tool.startsWith('python ')) {
      ...
      return agentConfig.allowedBashCommands?.some(allowed =>
        command.startsWith(allowed) || allowed === '*'
      );
    }
  ...
  isReadOnlyAgent(agentConfig) {
    return agentConfig.fil?.block?.includes('FIL-0') ||
  ...
  validateAgentConfig(config) {
    ...
    const hasPythonTools = config.tools?.some(t =>
    ...
    return {
      ...
      severity: issues.length > 0 ?
        Math.max(...issues.map(i => i.severity === 'critical' ? 3 : i.severity === 'error' ? 2 : 1)) :
        0,
      issues
    };
  }
  ...
  async generateCICheck(prDiff) {
    ...
    for (const usage of toolUsage) {
      ...
      const validation = await this.validateAgentTools(
        usage.agent,
        usage.tools,
        { files: usage.files }
      );
  ...
  hasLanguageSupport(config, language) {
    ...
    return patterns.tools.some(tool =>
