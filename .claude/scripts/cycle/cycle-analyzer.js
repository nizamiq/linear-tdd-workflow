...
class CycleAnalyzer {
  ...
  async analyzeCurrentCycle() {
    ...
    // Find active cycle
    const activeCycle = cycles.nodes.find(
      (c) => c.startsAt <= new Date() && c.endsAt >= new Date(),
    );
    ...
    console.log(
      `  ✓ Cycle ${activeCycle.number}: ${completed}/${total} issues (${this.metrics.cycle.progress.toFixed(1)}%)`,
    );
  }
  ...
  async calculateVelocity() {
    ...
    const completedCycles = cycles.nodes.filter((c) => c.endsAt < new Date()).slice(0, 3); // Last 3 completed
  ...
  async analyzeBacklog() {
    ...
    const byType = {
      bug: backlogIssues.filter((i) =>
        i.labels?.nodes.some((l) => l.name.toLowerCase().includes('bug')),
      ).length,
      feature: backlogIssues.filter((i) =>
        i.labels?.nodes.some((l) => l.name.toLowerCase().includes('feature')),
      ).length,
      techDebt: backlogIssues.filter((i) =>
        i.labels?.nodes.some(
          (l) => l.name.toLowerCase().includes('debt') || l.name.toLowerCase().includes('refactor'),
        ),
      ).length,
      other: 0,
    };
    ...
    // Calculate readiness
    const ready = backlogIssues.filter(
      (i) => i.estimate && i.description && i.description.length > 50,
    ).length;
  ...
  calculateDependencyRisk(blockers, dependencies) {
    const blockerScore = blockers.length * 10 + blockers.filter((b) => b.impact <= 2).length * 20;
