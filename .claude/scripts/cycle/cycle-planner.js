...
class CyclePlanner {
  ...
  async fetchCandidateIssues() {
    ...
    this.candidateIssues = issues.nodes.filter((issue) => {
      ...
      return (
        issue.estimate && // Must have estimate
  ...
  async scoreIssues() {
    ...
    this.scoredIssues.slice(0, 5).forEach((item) => {
      console.log(
        `    ${item.issue.identifier}: ${item.score.toFixed(2)} - ${item.issue.title.substring(0, 50)}`,
      );
    });
  }
  ...
  calculateTechnicalDebtImpact(issue) {
    ...
    const keywordMatches = debtKeywords.filter(
  ...
  async selectIssues() {
    ...
    for (const scoredIssue of this.scoredIssues) {
      ...
      console.log(
        `  + ${scoredIssue.issue.identifier} (${estimate}pts, score: ${scoredIssue.score.toFixed(1)})`,
      );
    ...
    console.log(
      `    Bugs: ${categoryCount.bug}, Tech Debt: ${categoryCount.tech_debt}, Features: ${categoryCount.feature}`,
    );
  }
  ...
  validateReadiness() {
    const notReady = this.selectedIssues.filter((si) => {
      ...
      return !issue.estimate || !issue.description || issue.description.length < 50;
    });
  ...
  async generateReport() {
    ...
    const report = {
      ...
      summary: {
        ...
        averageScore:
          this.selectedIssues.reduce((sum, si) => sum + si.score, 0) / this.selectedIssues.length,
      },
  ...
  getCategoryBreakdown() {
    ...
    Object.keys(breakdown).forEach((category) => {
      breakdown[category].percentage = ((breakdown[category].points / totalPoints) * 100).toFixed(
        1,
      );
    });
  ...
  planner
    ...
    .then((report) => {
      ...
      Object.entries(report.composition).forEach(([category, data]) => {
        console.log(
          `  ${category}: ${data.count} issues (${data.points} points, ${data.percentage}%)`,
        );
      });
