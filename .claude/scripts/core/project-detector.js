...
class ProjectDetector {
    ...
    generateWorkspaceRecommendations(data) {
        ...
        if (data.teams.nodes.length === 1) {
            recommendations.push(`Auto-detected team: ${data.teams.nodes[0].name} (${data.teams.nodes[0].key})`);
        ...
        if (data.projects.nodes.length > 0) {
            ...
            data.projects.nodes.forEach(project => {
                ...
                recommendations.push(`  - ${project.name} (Teams: ${teamNames}): LINEAR_PROJECT_ID=${project.id}`);
            });
        }
    ...
    async detectTaskPrefix(teamId) {
        ...
        try {
            ...
            const mostCommon = Array.from(prefixes.entries())
                .sort((a, b) => b[1] - a[1])[0];
    ...
    detectProjectType() {
        ...
        characteristics.hasTests = testDirs.some(dir =>
    ...
    recommendTaskPrefix(projectType) {
        if (projectType.frameworks.includes('react') || projectType.frameworks.includes('vue')) {
            return 'UI-';
        } else if (projectType.frameworks.includes('express') || projectType.frameworks.includes('fastapi')) {
            return 'API-';
        } else if (projectType.languages.includes('python')) {
            return 'PY-';
        } else if (projectType.languages.includes('javascript') || projectType.languages.includes('typescript')) {
            return 'JS-';
        } else {
            return 'TASK-';
        }
    ...
    async runSetupWizard() {
        ...
        console.log(`  Frameworks: ${recommendations.project.frameworks.join(', ') || 'None detected'}`);
        console.log(`  Test Frameworks: ${recommendations.project.testFrameworks.join(', ') || 'None detected'}`);
        ...
        console.log(`  Quality Threshold: ${recommendations.config.qualitySettings.duplicateThreshold}`);
