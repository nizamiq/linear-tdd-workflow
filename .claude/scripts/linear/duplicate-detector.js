...
class DuplicateDetector {
  ...
  normalizeText(text) {
    ...
    return text
      ...
      .split(/\s+/)
      .filter(
        (word) => word.length >= this.config.minWordLength && !this.config.stopWords.includes(word),
      )
      .join(' ');
  }
  ...
  findDuplicates(tasks) {
    ...
    if (this.config.verbose) {
      ...
      console.log(
        `📊 Thresholds: Title=${this.config.titleSimilarityThreshold}, Combined=${this.config.combinedThreshold}`,
      );
    }
  ...
  clusterDuplicates(duplicatePairs = this.duplicatePairs) {
    ...
    for (const pair of duplicatePairs) {
      ...
      const relatedPairs = duplicatePairs.filter(
        (p) =>
          (p.task1.id === task1Id ||
            p.task2.id === task1Id ||
            p.task1.id === task2Id ||
      ...
      summary: {
        ...
        avgConfidence:
          duplicatePairs.length > 0
            ? duplicatePairs.reduce((sum, p) => sum + p.confidence, 0) / duplicatePairs.length
            : 0,
        detectedAt: new Date().toISOString(),
      },
    ...
    return clusters
      .map((cluster) => {
        ...
        const primary = cluster.tasks.reduce((best, task) => {
          ...
          const bestScore = (best.description?.length || 0) + (best.comments?.length || 0) * 10;
...
if (require.main === module) {
  ...
  const options = {
    ...
    titleSimilarityThreshold:
      parseFloat(args.find((arg) => arg.startsWith('--title-threshold='))?.split('=')[1]) || 0.8,
    combinedThreshold:
      parseFloat(args.find((arg) => arg.startsWith('--combined-threshold='))?.split('=')[1]) || 0.7,
  };
  ...
  const mockTasks = [
    {
      ...
      description:
        'We need to add comprehensive unit tests for the user authentication module to ensure proper coverage.',
    ...
    },
    {
      ...
      description:
        'Add unit test coverage for user authentication functionality to meet quality standards.',
      state: { name: 'To Do' },
    ...
    },
    {
      ...
      description:
        'Database connections are not being properly pooled, causing performance issues.',
