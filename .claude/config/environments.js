...
function getWebhookUrl(environment) {
  ...
  const baseUrl =
    process.env.WEBHOOK_BASE_URL || process.env[`WEBHOOK_BASE_URL_${environment.toUpperCase()}`];
...
const environments = {
  development: {
    ...
    fileProcessing: {
      ...
      ignoredPatterns: (
        process.env.DEV_IGNORED_PATTERNS ||
  ...
  staging: {
    ...
    fileProcessing: {
      ...
      ignoredPatterns: (
        process.env.STAGING_IGNORED_PATTERNS ||
  ...
  production: {
    ...
    fileProcessing: {
      ...
      ignoredPatterns: (
        process.env.PROD_IGNORED_PATTERNS ||
    ...
    security: {
      ...
      allowedOrigins: (process.env.PROD_ALLOWED_ORIGINS || 'https://app.linear.app')
        .split(',')
        .map((o) => o.trim()),
    },
    ...
    monitoring: {
      ...
      alerting: {
        ...
        escalationLevels: (process.env.PROD_ESCALATION_LEVELS || 'warning,critical,emergency')
          .split(',')
          .map((l) => l.trim()),
      },
    },
