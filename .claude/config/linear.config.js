...
class LinearConfig {
  constructor() {
    ...
    this.required = ['LINEAR_API_KEY', 'LINEAR_TEAM_ID'];
    this.optional = ['LINEAR_PROJECT_ID', 'LINEAR_TASK_PREFIX', 'LINEAR_WEBHOOK_SECRET'];
  }
  ...
  getRequired(key) {
    ...
    if (!value) {
      throw new Error(
        `Required environment variable ${key} is not set. Please configure your Linear integration.`,
      );
    }
    return value;
  }
  ...
  validateConfig(config) {
    ...
    if (config.tasks.prefix && !/^[A-Z]+-?$/.test(config.tasks.prefix)) {
      errors.push(
        'LINEAR_TASK_PREFIX must be uppercase letters followed by optional dash (e.g., TASK-, BUG-, DEV-)',
      );
    }
    ...
    if (
      config.quality.titleSimilarityThreshold < 0 ||
      config.quality.titleSimilarityThreshold > 1
    ) {
  ...
  getConfigurationRecommendations(missing, optional) {
    ...
    if (!optional.LINEAR_TASK_PREFIX) {
      recommendations.push(
        'Set LINEAR_TASK_PREFIX to customize task naming (e.g., DEV-, BUG-, TASK-)',
      );
    }
