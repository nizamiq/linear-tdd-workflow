...
class GitHooksInstaller {
   ...
   */
  async installHooks() {
    console.log('🔧 Installing TDD enforcement git hooks...');
...
echo "🔍 TDD Gate Enforcer: Pre-commit validation..."

# Check if Node.js is available
if ! command -v node &> /dev/null; then
    echo "❌ Node.js is required for TDD enforcement"
    exit 1
fi
...
echo "🚀 TDD Gate Enforcer: Pre-push validation..."
...
echo "✅ TDD Gate Enforcer: Pre-push validation passed"
`;
  }
...
if echo "$commit_msg" | grep -qE "\\[(RED|GREEN|REFACTOR)\\]"; then
    echo "🔍 TDD phase detected in commit message"
fi
...
if echo "$commit_msg" | grep -qE "\\b(TODO|FIXME)\\b"; then
    if ! echo "$commit_msg" | grep -qE "\\[allow-(todo|fixme)\\]"; then
        echo "❌ Commit message contains TODO or FIXME"
        echo "💡 Add [allow-todo] or [allow-fixme] if intentional"
        exit 1
    fi
fi
...
`;
  ...
  async createTDDConfig() {
    ...
    if (fs.existsSync(configPath)) {
      console.log('ℹ️ TDD config already exists, skipping creation');
      return;
    }
    ...
    console.log('✅ TDD configuration created at .claude/config/tdd-config.json');
  }
  ...
  async uninstallHooks() {
    console.log('🗑️ Removing TDD enforcement hooks...');
    ...
    for (const hook of hooks) {
      ...
      if (fs.existsSync(hookPath)) {
        ...
        if (content.includes('Claude Agentic Workflow System')) {
          ...
          console.log(`⚠️ ${hook} hook exists but not created by us, skipping`);
        }
      }
    }

    console.log('✅ TDD enforcement hooks removed');
  }
   ...
   */
  checkInstallation() {
    console.log('🔍 Checking TDD hook installation status...');
   ...
   */
  async testHooks() {
    console.log('🧪 Testing TDD enforcement hooks...');
    ...
    try {
      ...
      console.log('🔍 Testing TDD gate enforcer...');
